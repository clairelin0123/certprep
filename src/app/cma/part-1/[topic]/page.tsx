...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const topic = await getTopicWithQuestions("cma", "part-1", topicSlug);
  if (!topic) return { title: "Topic Not Found" };

  return {
    title: `${topic.name} — CMA Part 1 Practice Questions (Free)`,
    description: `${topic.questions.length} free practice questions for ${topic.name} with detailed explanations. CMA Part 1: Financial Planning, Performance & Analytics. ${topic.weight}% exam weight.`,
    alternates: { canonical: `/cma/part-1/${topicSlug}` },
  };
}
...
export default async function TopicPage({ params }: Props) {
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Part 1", item: `${siteUrl}/cma/part-1` },
      { "@type": "ListItem", position: 4, name: topic.name, item: `${siteUrl}/cma/part-1/${topicSlug}` },
    ],
  };
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <div className="mb-2 text-sm text-gray-500">
        ...
        &gt;{" "}
        <Link href="/cma/part-1" className="hover:text-blue-600">
          Part 1
        </Link>{" "}
        &gt; {topic.name}
      </div>
      ...
      <p className="mt-2 text-gray-600">
        {topic.weight}% of the CMA Part 1 exam &middot; {questions.length}{" "}
        practice questions
      </p>

      <div className="mt-4">
        <Link
          href={`/cma/part-1/${topicSlug}/quiz`}
